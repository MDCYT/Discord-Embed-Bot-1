...
module.exports = {
    ...
    async execute(client, member, interaction, guild, channel, fields, bedrock) {
        const data = {
            ...
            ServerPort: isNaN(Number(fields.get("port").value)) ? (bedrock ? process.env.BDPORT : process.env.JAVAPORT) : Number(fields.get("port").value),
       ...
       await interaction.deferReply({ephemeral: true});
        ...
        if (process.env.si !== 'si') // si si, es si :sob:, weno, si si es... si... Deberia ignorarlo
        ...
        .setLabel("Aceptar")
        .setCustomId("acceptMinecraftServer="+data.ServerDomain+","+data.ServerPort+","+message.id +","+message.channel.id)
        .setStyle(ButtonStyle.Success)
        .setEmoji("✅");
        const buttonToReject = new ButtonBuilder()
        .setLabel("Rechazar")
        .setCustomId("rejectMinecraftServer="+data.ServerDomain+","+data.ServerPort +","+message.id+","+message.channel.id)
        .setStyle(ButtonStyle.Danger)
        .setEmoji("❌");
        const row = new ActionRowBuilder()
        .addComponents(buttonToAccept, buttonToReject);
        message.edit({
            content: "",
            embeds: [embed],
            components: [row]
    ...
    async accept(ip, port, guild) {
        const serverConfig = await MinecraftConfig.selectMinecraftConfigRow(guild.id);
        const channel = guild.channels.cache.get(serverConfig[0].ChannelID);
        const data = await MinecraftConfig.selectMinecraftServerRow(guild.id, ip, port);
    ...
    async delete(ip, port, guild) {
        ...
        return {content: "Se rechazo correctamente el servidor.", user: data.User};
    }
