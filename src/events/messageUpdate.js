	...
	async execute(oldMessage, newMessage, commands, client) {
      ...
      !oldMessage.command
        ...
        messageDeleteLog.permissionsFor(newMessage.guild.me).has(['SEND_MESSAGES', 'EMBED_LINKS'])
				...
				if (oldMessage.embeds.length > 1) {embed.setDescription(`${newMessage.member}'s **message embeds** in ${newMessage.channel} were deleted.`);}
				else {embed.setDescription(`${newMessage.member}'s **message embed** in ${newMessage.channel} was deleted.`);}
