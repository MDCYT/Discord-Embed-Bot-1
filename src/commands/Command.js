...
class Command {
   ...
   */
	checkPermissions(message, ownerOverride = true) {
		...
		if (!message.guild.members.me.permissionsIn(message.channel).has([PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks])) return false;
   ...
   */
	checkUserPermissions(message, ownerOverride = true) {
      ...
      message.member.permissionsIn(message.channel).missing(this.userPermissions).map(p => permissions[p]);
   ...
   */
	checkClientPermissions(message) {
		const missingPermissions =
    message.guild.members.me.permissionsIn(message.channel).missing(this.clientPermissions).map(p => permissions[p]);
		if (missingPermissions.length !== 0) {
			const embed = new EmbedBuilder()
				.setAuthor({
					name: `${message.author.tag}`,
   ...
   */
	static validateOptions(client, options) {
		...
		// Aliases
		if (options.aliases) {
			if (!Array.isArray(options.aliases) || options.aliases.some(ali => typeof ali !== 'string')) {throw new TypeError('Command aliases is not an Array of strings');}

			if (options.aliases.some(ali => ali !== ali.toLowerCase())) {throw new RangeError('Command aliases are not lowercase');}
		...
		if (options.type && !Object.values(client.types).includes(options.type)) {throw new Error('Command type is not valid');}

		// Client permissions
		if (options.clientPermissions) {
			if (!Array.isArray(options.clientPermissions)) {throw new TypeError('Command clientPermissions is not an Array of permission key strings');}
		...
		if (options.userPermissions) {
			if (!Array.isArray(options.userPermissions)) {throw new TypeError('Command userPermissions is not an Array of permission key strings');}
		...
		// Examples
		if (options.examples && !Array.isArray(options.examples)) {throw new TypeError('Command examples is not an Array of permission key strings');}
		...
		// Cooldown
		if (options.cooldown) {
			if (typeof options.cooldown !== 'number') {throw new TypeError('Command cooldown is not a number');}
			if (options.cooldown < 0) {throw new RangeError('Command cooldown must be a positive number');}
		}
